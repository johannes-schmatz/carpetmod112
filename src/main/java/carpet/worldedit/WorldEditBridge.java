...
/**
 * WorldEdit bridge class, adds an extra layer of abstraction so classes from WorldEdit itself are not attempted to be loaded when WorldEdit is not present.
 */
public class WorldEditBridge {
	...
	public static void onServerLoaded(MinecraftServer server) {
		if (worldEditPresent) CarpetWorldEdit.inst.onServerLoaded(server);
	}

	public static void onStartTick() {
		if (worldEditPresent) CarpetWorldEdit.inst.onStartTick();
	}

	public static void onCommand(Command command, CommandSource sender, String[] args) {
		if (worldEditEnabled()) CarpetWorldEdit.inst.onCommand(command, sender, args);
	}

	public static boolean onLeftClickBlock(World world, BlockPos pos, ServerPlayerEntity player) {
		if (worldEditEnabled()) return CarpetWorldEdit.inst.onLeftClickBlock(world, pos, player);
		else return true;
	}

	public static boolean onRightClickBlock(World world, BlockPos pos, ServerPlayerEntity player) {
		if (worldEditEnabled()) return CarpetWorldEdit.inst.onRightClickBlock(world, pos, player);
		else return true;
	}

	public static boolean onRightClickAir(World world, ServerPlayerEntity player) {
		if (worldEditEnabled()) return CarpetWorldEdit.inst.onRightClickAir(world, player);
		else return true;

	}

	public static void onCustomPayload(CustomPayloadC2SPacket packet, ServerPlayerEntity player) {
		if (worldEditEnabled()) WECUIPacketHandler.onCustomPayload(packet, player);
	}

	public static void startEditSession(ServerPlayerEntity player) {
		if (worldEditEnabled()) CarpetWorldEdit.inst.startEditSession(player);
	}

	public static void finishEditSession(ServerPlayerEntity player) {
		if (worldEditEnabled()) CarpetWorldEdit.inst.finishEditSession(player);
	}
	...
	public static void recordBlockEdit(ServerPlayerEntity player, World world, BlockPos pos, BlockState newBlock, NbtCompound newTileEntity) {
		if (worldEditEnabled()) CarpetWorldEdit.inst.recordBlockEdit(player, world, pos, newBlock, newTileEntity);
	}

	public static void recordEntityCreation(ServerPlayerEntity player, World world, Entity created) {
		if (worldEditEnabled()) CarpetWorldEdit.inst.recordEntityCreation(player, world, created);
	}

	public static void recordEntityRemoval(ServerPlayerEntity player, World world, Entity removed) {
		if (worldEditEnabled()) CarpetWorldEdit.inst.recordEntityRemoval(player, world, removed);
	}
