...
class CarpetWorldEdit {
	...
	public void onCommand(Command command, CommandSource sender, String[] args) {
	...
	public boolean onLeftClickBlock(World world, BlockPos pos, ServerPlayerEntity player) {
		if (!platform.isHookingEvents()) return true;
		...
		if (we.handleBlockLeftClick(carpetPlayer, vector)) result = false;

		if (we.handleArmSwing(carpetPlayer)) result = false;
	...
	public boolean onRightClickBlock(World world, BlockPos pos, ServerPlayerEntity player) {
		...
		if (we.handleBlockRightClick(carpetPlayer, vector)) result = false;

		if (we.handleRightClick(carpetPlayer)) result = false;
	...
	public boolean onRightClickAir(World world, ServerPlayerEntity player) {
		...
		if (we.handleRightClick(carpetPlayer)) return false;
	...
	public void finishEditSession(ServerPlayerEntity player) {
		...
		if (sessionNestedDepth == 0) {
			CarpetPlayer carpetPlayer = wrap(player);
			if (editSession.getChangeSet().size() > 0) WorldEdit.getInstance().getSessionManager().get(carpetPlayer).remember(editSession);
			editSession = null;
		}
	}

	public void recordBlockEdit(ServerPlayerEntity player, World world, BlockPos pos, BlockState newBlock, NbtCompound newTileEntity) {
		if (player == null) return;
		...
		BaseBlock previous;
		if (oldTileEntity == null) previous = new BaseBlock(oldBlockId, oldMeta);
		else previous = new BaseBlock(oldBlockId, oldMeta, NBTConverter.fromNative(oldTileEntity.writeNbt(new NbtCompound())));

		int newBlockId = Block.getId(newBlock.getBlock());
		int newMeta = newBlock.getBlock().getDropItemMetadata(newBlock);
		BaseBlock current;
		if (newTileEntity == null) current = new BaseBlock(newBlockId, newMeta);
		else current = new BaseBlock(newBlockId, newMeta, NBTConverter.fromNative(newTileEntity));
