...
public class RandomTickOptimization {
	...
	static {
		for (Block b : Block.REGISTRY) {
			if (b instanceof AbstractPressurePlateBlock || b instanceof ButtonBlock || b instanceof PumpkinBlock || b instanceof RedstoneTorchBlock) {
	...
	public static void recalculateAllChunks() {
		...
		for (World world : server.worlds) {
			...
			if (!(provider instanceof ServerChunkCache)) continue;
			for (WorldChunk chunk : ((ServerChunkProviderAccessor) provider).getLoadedChunksMap().values()) {
				for (WorldChunkSection subchunk : chunk.getSections()) {
					if (subchunk != null) subchunk.validateBlockCounters();
				}
			}
		}
	}
