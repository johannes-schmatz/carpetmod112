...
public class OptimizedTNT {
	...
	public static void doExplosionA(ExplosionAccessor e) {
		...
		if (vec3dmem == null || !vec3dmem.equals(vec3d) || tickmem != e.getWorld().getTime()) {
			...
			entitylist = e.getWorld().getEntities((Entity) null, new Box(k1, i2, j2, l1, i1, j1));
			explosionSound = 0;
		}
		...
		for (int k2 = 0; k2 < entitylist.size(); ++k2) {
			...
			if (entity instanceof PrimedTntEntity && entity.x == e.getEntity().x && entity.y == e.getEntity().y && entity.z == e.getEntity().z) {
				continue;
			}

			if (!entity.isImmuneToExplosions()) {
				double d12 = entity.getDistanceTo(e.getX(), e.getY(), e.getZ()) / (double) f3;
				...
				if (d12 <= 1.0D) {
					...
					if (d13 != 0.0D) {
						...
						pairMutable.setRight(entity.getShape());
						density = densityCache.getOrDefault(pairMutable, Double.MAX_VALUE);

						if (density == Double.MAX_VALUE) {
							Pair<Vec3d, Box> pair = Pair.of(vec3d, entity.getShape());
							density = e.getWorld().getBlockDensity(vec3d, entity.getShape());
							densityCache.put(pair, density);
						}

						double d10 = (1.0D - d12) * density;
						entity.damage(DamageSource.explosion((Explosion) e), (float) ((int) ((d10 * d10 + d10) / 2.0D * 7.0D * (double) f3 + 1.0D)));
						double d11 = d10;

						if (entity instanceof LivingEntity) {
							d11 = ProtectionEnchantment.modifyExplosionDamage((LivingEntity) entity, d10);
						}
	...
	public static void doExplosionB(ExplosionAccessor e, LogableExplosion e1, boolean spawnParticles) {
		...
		if (explosionSound < 100 || explosionSound % 100 == 0) {
			...
			if (e.getPower() >= 2.0F && e.getDamagesTerrain()) {
				world.addParticle(ParticleType.EXPLOSION_HUGE, posX, posY, posZ, 1.0D, 0.0D, 0.0D);
			} else {
				world.addParticle(ParticleType.EXPLOSION_LARGE, posX, posY, posZ, 1.0D, 0.0D, 0.0D);
			}
		}
		...
		if (e.getDamagesTerrain()) {
			for (BlockPos blockpos : e.getAffectedBlocks()) {
				...
				if (spawnParticles) {
					...
					d4 = d4 * d7;
					d5 = d5 * d7;
					world.addParticle(ParticleType.EXPLOSION_NORMAL, (d0 + posX) / 2.0D, (d1 + posY) / 2.0D, (d2 + posZ) / 2.0D, d3, d4, d5);
				...
				if (iblockstate.getMaterial() != Material.AIR) {
					if (block.shouldDropItemsOnExplosion((Explosion) e)) {
						...
						block.dropItems(world, blockpos, iblockstate, 1.0F / e.getPower(), 0);
					}

					world.setBlockState(blockpos, Blocks.AIR.defaultState(), 3);
					block.onExploded(world, blockpos, (Explosion) e);
				}
			}
		}
		...
		if (e.getCreateFire()) {
			for (BlockPos blockpos1 : e.getAffectedBlocks()) {
				...
				WorldChunk chunk = world.getChunkAt(blockpos1.getX() >> 4, blockpos1.getZ() >> 4);

				if (chunk.getBlockState(blockpos1).getMaterial() == Material.AIR && chunk.getBlockState(blockpos1.down()).isFullBlock() &&
		...
		if (LoggerRegistry.__explosions) {
			e1.getLogHelper().onExplosionDone(e.getWorld().getTime());
		}
	}

	private static void removeFast(List<Entity> lst, int index) {
		if (index < lst.size() - 1) lst.set(index, lst.get(lst.size() - 1));
				...
				for (int l = 0; l < 16; ++l) {
					if (j == 0 || j == 15 || k == 0 || k == 15 || l == 0 || l == 15) {
						...
						for (float f1 = 0.3F; f > 0.0F; f -= 0.22500001F) {
							...
							if (iblockstate.getMaterial() != Material.AIR) {
								float f2 = e.getEntity() != null ? e.getEntity().getBlastResistance((Explosion) e, e.getWorld(), blockpos, iblockstate) :
										iblockstate.getBlock().getBlastResistance(null);
								f -= (f2 + 0.3F) * 0.3F;
							}

							if (f > 0.0F && (e.getEntity() == null || e.getEntity().canExplodeBlock((Explosion) e, e.getWorld(), blockpos, iblockstate, f))) {
	...
	private static boolean checkAffectedPosition(ExplosionAccessor e, double xRel, double yRel, double zRel) {
		...
		for (float f1 = 0.3F; size > 0.0F; size -= 0.22500001F) {
			...
			if (size > 0.0F && (e.getEntity() == null || e.getEntity().canExplodeBlock((Explosion) e, e.getWorld(), posMutable, state, size))) {
	...
	private static void showTNTblastChance(ExplosionAccessor e) {
		...
		for (PlayerEntity player : e.getWorld().players) {
			Messenger.m(player,
					"w Pop: ",
					"c " + nf.format(chance) + " ",
