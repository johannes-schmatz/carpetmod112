...
public abstract class TickingArea {
	...
	public static void saveConfig(MinecraftServer server) {
		for (World world : server.worlds) {
			ChunkSource chunkProvider = world.getChunkSource();
			if (!(chunkProvider instanceof ServerChunkCache)) continue;
			ChunkStorage chunkLoader = ((ServerChunkProviderAccessor) chunkProvider).getChunkLoader();
			if (!(chunkLoader instanceof AnvilChunkStorage)) continue;
	...
	public static void initialChunkLoad(MinecraftServer server, boolean log) {
		for (ServerWorld world : server.worlds) {
			if (TickingArea.hasTickingArea(world)) {
				if (log) LOGGER.info("[CM]: Preparing start region for level {}", world.dimension.getType().getKey());
				...
				for (ChunkPos chunk : chunksToLoad) {
					...
					if (time - lastTime > 1000) {
						if (log) ((MinecraftServerAccessor) server).invokeLogPercentage("[CM]: Preparing spawn area", count * 100 / chunksToLoad.size());
						lastTime = time;
					}
	...
	public static class SpawnChunks extends TickingArea {
		...
		public boolean readFromConfig(String[] args) {
			if (args.length > 1) setName(Arrays.stream(args).skip(1).collect(Collectors.joining(" ")));
			return true;
		}
	}
	...
	public static class Square extends TickingArea {
		...
		@Override
		protected boolean contains(World world, int chunkX, int chunkZ) {
			return chunkX >= min.x && chunkX <= max.x && chunkZ >= min.z && chunkZ <= max.z;
		}
		...
		public boolean readFromConfig(String[] args) {
			...
			if (args.length > 5) setName(Arrays.stream(args).skip(5).collect(Collectors.joining(" ")));
	...
	public static class Circle extends TickingArea {
		...
		public boolean readFromConfig(String[] args) {
			...
			if (args.length > 4) setName(Arrays.stream(args).skip(4).collect(Collectors.joining(" ")));
