...
public class Waypoint implements Comparable<Waypoint> {
	...
	public String getFullName() {
		return getDimension().getKey() + ":" + name;
	}
	...
	public int compareTo(Waypoint other) {
		...
		if (this.world == other.world) return name.compareTo(other.name);
	...
	public void teleport(Entity entity) {
		if (entity.world != this.world) {
			...
			worldFrom.getChunkAt(entity.chunkX, entity.chunkZ).removeEntity(entity, entity.chunkY);
			entity.removed = false;
			entity.refreshPositionAndAngles(x, y, z, (float) yaw, (float) pitch);

			if (entity.isAlive()) {
				worldTo.addEntity(entity);
				worldTo.tickEntity(entity, false);
			}
			...
			((ServerPlayerEntity) entity).networkHandler.teleport(x, y, z, (float) yaw, (float) pitch);
		} else {
			entity.refreshPositionAndAngles(x, y, z, (float) yaw, (float) pitch);
		}
	}
	...
	public static File getWaypointFile(World world) {
		...
		return new File(world.getStorage().getDir(), filename);
	}

	public static Gson getGson(MinecraftServer server) {
		return new GsonBuilder().registerTypeAdapter(CollectionAdapter.type, new CollectionAdapter(server)).setPrettyPrinting().create();
	}
	...
	public static class CollectionAdapter extends TypeAdapter<Collection<Waypoint>> {
		public static final Type type = new TypeToken<Collection<Waypoint>>() {
		}.getType();
		...
		public void write(JsonWriter out, Collection<Waypoint> waypoints) throws IOException {
			...
			for (Waypoint w : waypoints) {
				out.name(w.name);
				out.beginObject();
				out.name("dimension").value(w.world.dimension.getType().getKey());
		...
		public Collection<Waypoint> read(JsonReader in) throws IOException {
			...
			while (in.hasNext()) {
				...
				while (in.hasNext()) {
					switch (in.nextName()) {
						case "dimension": {
							world = server.getWorld(DimensionType.byKey(in.nextString()).getId());
							break;
						}
						case "x": {
							x = in.nextDouble();
