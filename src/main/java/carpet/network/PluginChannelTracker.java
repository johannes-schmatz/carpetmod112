...
public class PluginChannelTracker {
	...
	/**
	 * Returns the collection of players registered to {@code channel}. The {@code server} is used to look players up by their name.
	 */
	public Set<ServerPlayerEntity> getPlayers(String channel) {
		PlayerManager pl = server.getPlayerManager();
		return channel2names.get(channel).stream().map(pl::get).collect(Collectors.toSet());
	}
