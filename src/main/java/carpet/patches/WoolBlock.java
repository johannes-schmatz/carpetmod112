...
public class WoolBlock extends net.minecraft.block.ColoredBlock {
	...
	private final Map<DyeColor, Set<Pair<Integer, BlockPos>>> woolBlocks = new EnumMap<>(DyeColor.class);
	private final EnumSet<DyeColor> alreadyCheckedColors = EnumSet.noneOf(DyeColor.class);
	private boolean updatingWool;
	private final Set<Pair<Integer, BlockPos>> updatedBlocks = new HashSet<>();
	...
	public void neighborChanged(BlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
		...
		if (!updatedBlocks.add(Pair.of(worldIn.dimension.getType().getId(), pos))) return;
	...
	public int getSignal(BlockState state, WorldView blockAccess, BlockPos pos, Direction side) {
		if (!CarpetSettings.wirelessRedstone) return 0;

		World worldIn = (World) blockAccess;
		// Adds this location if absent
		woolBlocks.get(state.get(COLOR)).add(Pair.of(worldIn.dimension.getType().getId(), pos));

		if (!alreadyCheckedColors.add(state.get(COLOR))) return 0;
		...
		for (Pair<Integer, BlockPos> location : getAllWoolOfType(worldIn.getServer(), state.get(COLOR))) {
			...
			for (Direction facing : Direction.values()) {
				...
				if (world.getBlockState(testPos) != state) power = Math.max(power, world.getSignal(testPos, facing));
			}
			CarpetClientChunkLogger.resetReason();
		}
