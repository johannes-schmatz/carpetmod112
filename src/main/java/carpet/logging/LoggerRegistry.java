...
public class LoggerRegistry {
	...
	public static void initLoggers(MinecraftServer server) {
		...
		registerLogger("counter", new Logger(server,
				"counter",
				"white",
				new String[]{"all", "cactus", "white", "orange", "magenta", "light_blue", "yellow", "lime", "pink", "gray", "silver", "cyan", "purple", "blue",
		...
		try {
			...
			for (Map.Entry<String, Map<String, LoggerOptions>> playerEntry : playerSubscriptions.entrySet()) {
				...
				for (LoggerOptions logger : playerEntry.getValue().values()) {
					playerLoggers.add(logger.toJson());
				}
	...
	public static boolean setDefault(MinecraftServer server, String logName, String option, LogHandler handler) {
		...
		for (PlayerEntity player : server.getPlayerManager().getAll()) {
			if (!hasSubscriptions(player.getName())) subscribePlayer(player.getName(), logName, option, handler);
		}
		return true;
	}
	...
	public static boolean removeDefault(MinecraftServer server, String logName) {
		if (defaultSubscriptions.containsKey(logName)) {
			...
			for (PlayerEntity player : server.getPlayerManager().getAll()) {
				if (!hasSubscriptions(player.getName())) unsubscribePlayer(player.getName(), logName);
			}
			return true;
		}
		return false;
	...
	/**
	 * Unsubscribes a player from all logs and removes the subscription entry This restores the default subscriptions
	 */
	public static void resetSubscriptions(MinecraftServer server, String playerName) {
		...
		if (hasSubscriptions(playerName)) playerSubscriptions.remove(playerName);
		...
		for (LoggerOptions option : defaultSubscriptions.values()) {
			...
			if (option.handlerName != null) handler = LogHandler.createHandler(option.handlerName, option.extraArgs);
