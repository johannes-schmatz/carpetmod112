...
public class DamageReporter {
	private static Text[] verifyAndProduceMessage(String option, PlayerEntity player, Entity from, Entity to, Supplier<Text> messageFuture) {
		if ("all".equalsIgnoreCase(option) || ("players".equalsIgnoreCase(option) && (from instanceof PlayerEntity || to instanceof PlayerEntity)) ||
	...
	public static boolean register_damage_attacker(Entity target, LivingEntity source, float amount) {
		...
		LoggerRegistry.getLogger("damage")
				.logNoCommand((option, player) -> verifyAndProduceMessage(option,
						player,
						source,
						target,
						() -> Messenger.m(null,
	...
	public static void register_damage(LivingEntity target, DamageSource source, float amount) {
		...
		if (source.isFire() && (target.isImmuneToFire() || target.hasStatusEffect(StatusEffects.FIRE_RESISTANCE))) return;
		LoggerRegistry.getLogger("damage")
				.logNoCommand((option, player) -> verifyAndProduceMessage(option, player, source.getAttacker(), target, () -> Messenger.m(null,
						target.getName(),
						"g  receiving ",
						String.format("r %.2f", amount),
	...
	public static void register_final_damage(LivingEntity target, DamageSource source, float amount) {
		if (!LoggerRegistry.__damage) return;
		LoggerRegistry.getLogger("damage")
				.log((option, player) -> verifyAndProduceMessage(option,
								player,
								source.getAttacker(),
								target,
								() -> Messenger.m(null, "g  - total received ", String.format("r %.2f", amount), "g  points of damage")
						),
						"ATTACKER",
						source.getAttacker() == null ? null : source.getAttacker().getScoreboardName(),
						"TARGET",
	...
	public static void modify_damage(LivingEntity target, DamageSource source, float previous_amount, float final_amount, String component) {
		if (!LoggerRegistry.__damage) return;
		if (previous_amount == final_amount) return;
		if (source.isFire() && (target.isImmuneToFire() || target.hasStatusEffect(StatusEffects.FIRE_RESISTANCE))) return;
		LoggerRegistry.getLogger("damage").logNoCommand((option, player) -> verifyAndProduceMessage(option, player, source.getAttacker(), target, () -> {
