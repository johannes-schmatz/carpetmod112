...
public class InstantComparators {
	...
	public static void onNoTileEntity(World world, BlockPos pos) {
		if (LoggerRegistry.__instantComparators) {
			LoggerRegistry.getLogger("instantComparators")
					.log(option -> new Text[]{Messenger.m(null, "y Comparator has no tile entity ", Messenger.tp("y", pos))
		...
		if (LoggerRegistry.__instantComparators) {
			LoggerRegistry.getLogger("instantComparators").log(option -> {
				if ((!buggy && !"buggy".equals(option)) || (buggy && !"tileTick".equals(option))) {
					return new Text[]{Messenger.m(null, "l " + (buggy ? "Buggy" : "Tile tick") + " instant comparator detected ", Messenger.tp("y", pos))
					};
				} else {
					return null;
				}
			}, "X", pos.getX(), "Y", pos.getY(), "Z", pos.getZ(), "BUGGY", buggy);
		}
	}
