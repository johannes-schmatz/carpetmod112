...
public class ExplosionLogHelper {
	...
	private static boolean affectBlocks = false; // TODO: will be used later when I add in the full explosion logger
	...
	public void onExplosionDone(long gametime) {
		if(lastGametime != gametime) {
			...
			LoggerRegistry.getLogger("explosions").log((option) -> {
				return new Text[]{Messenger.m(null, "wb tick : ", "d " + gametime)};
			});
		}
		...
		LoggerRegistry.getLogger("explosions").log((option) -> {
			...
			switch (option) {
				...
				// TODO: temporarily removed "full" because its not really needed for my use case. may implement later - Sylkos
