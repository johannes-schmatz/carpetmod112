...
public class CommandUnload13 extends CommandCarpetBase
{
    ...
    public void run(MinecraftServer server, CommandSource sender, String[] args) throws CommandException
    {
        ...
        if (order)
        {
            List<String> orders = ChunkLoading.check_unload_order_13((ServerWorld)sender.getSourceWorld(), pos, pos2);
            print_multi_message(orders, sender);
            return;
        }
        if (protect)
        {
            List<String> orders = ChunkLoading.protect_13((ServerWorld)sender.getSourceWorld(), pos, pos2, args[0]);
            print_multi_message(orders, sender);
            return;
        }
    ...
    public List<String> getSuggestions(MinecraftServer server, CommandSource sender, String[] args, @Nullable BlockPos pos)
    {
        ...
        if (args.length == 1)
        {
            return suggestMatching(args, "verbose", "brief", "order", "nether", "overworld", "end", "protect");
        }
