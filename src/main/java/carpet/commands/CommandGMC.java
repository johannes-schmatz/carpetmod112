    ...
    public void run(MinecraftServer server, CommandSource sender, String[] args) throws CommandException {
        ...
        else
        {
            ...
            if(CarpetSettings.cameraModeSurvivalRestrictions && entityplayer.interactionManager.getGameMode() == GameMode.SURVIVAL) {
                List<HostileEntity> hostiles = sender.getSourceWorld().getEntities(HostileEntity.class, new Box(entityplayer.x - 8.0D, entityplayer.y - 5.0D,
                        entityplayer.z - 8.0D, entityplayer.x + 8.0D, entityplayer.y + 5.0D, entityplayer.z + 8.0D), mob -> mob.m_9015927(entityplayer));
                StatusEffectInstance fireresist = entityplayer.getEffectInstance(StatusEffect.get("fire_resistance"));
                if(!entityplayer.onGround || entityplayer.m_1567581() || (((EntityAccessor) entityplayer).getFireTicks() > 0 && (fireresist == null || fireresist.getDuration() < ((EntityAccessor) entityplayer).getFireTicks())) || entityplayer.getBreath() != 300 || !hostiles.isEmpty()){
