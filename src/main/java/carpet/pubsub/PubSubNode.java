...
/**
 * A node in the PubSub tree Classes outside this package should interact with {@link PubSubManager} passing nodes as arguments
 */
public final class PubSubNode {
	...
	/**
	 * Keeps track of the number of subscribers in this branch of the tree (root -> subscriber) Used for only updating values if there are actually subscribers
	 * that would get them
	 */
	...
	@Nullable PubSubNode getChildNode(String... path) {
		return getChildNode(path, 0, false);
	}

	@Nullable PubSubNode getChildNode(Collection<String> path) {
	...
	/**
	 * Add a subscriber to this node Updates {@link #totalSubscriberCount} for all parents and children
	 *
	...
	/**
	 * Updates children after subscribing. Called recursively and initially by {@link #subscribe(PubSubSubscriber)}
	 *
	 * @param subscriber The subscriber being added
	 */
	...
	/**
	 * Remove a subscriber from this node Updates {@link #totalSubscriberCount} for all parents and children
	 *
	...
	/**
	 * Updates children after unsubscribing. Called recursively and initially by {@link #unsubscribe(PubSubSubscriber)}
	 *
	 * @param subscriber The subscriber being removed
	 */
	...
	/**
	 * Instruct this node to update values from providers (called every tick). May already be suppressed by the parent if there are no subscribers.
	 *
	...
	public boolean equals(Object obj) {
		return obj != null && obj.getClass() == PubSubNode.class && Objects.equals(((PubSubNode) obj).parent, this.parent) &&
